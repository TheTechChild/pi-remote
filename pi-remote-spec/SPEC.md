...
| D3 | macOS suspend detection | cgo binding to IOKit (`IORegisterForSystemPower`): unlike the originally-sketched `NSWorkspace` notifications, it works without a GUI app run loop (daemon-appropriate) and lets the daemon acknowledge sleep only after `machine_suspending` is sent, guaranteeing the pre-sleep send window. Implementation in `internal/suspend/suspend_darwin.go` behind `//go:build darwin && cgo`. Fallback non-suspend stub for other platforms (and cgo-less darwin builds) in the same package. |
